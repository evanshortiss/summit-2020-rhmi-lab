...
* Access to the Apicurito API designer tool for exploring the OpenAPI Spec compatible API definition
...
. Open the link:{apicurio-url}[Apicurito Dashboard] and click the *OpenAPI* button.
. When prompted to choose a file select the JSON file you just unzipped.

[type=verification]
Did the Apicurito UI render the OpenAPI Spec using the file you uploaded?

[type=verificationFail]
Verify that you selected the correct JSON file. If the OpenAPI Spec fails to render in Apicurito please contact the lab administrator.

=== View API Data Types

This API contains two primary data types, the *Junction* and *Meter*. Here's how you can view their properties using Apicurito.

. On the left-hand side of the Apicurito UI you should see a *Data Types* section after the OpenAPI Spec JSON was loaded.
. Choose the *Junction* item under the *Data Types* heading. Information and properties of a *Junction* are now displayed on the right of the Apicurito UI.
...
This information will be provided to your API server via a *Secret* you will create in the OpenShift UI.

=== Create an OpenShift Project

. Login to the link:{openshift-console-url}[OpenShift Console].
. Choose *Home > Projects* from the side menu.
. Click the *Create Project* button.
.. Enter `{user-project-name}` in the *Name* field.
.. Enter `{user-sanitized-username} RHMI Lab` in the *Display Name* field.
.. Click *Create*.
...
.. For *Image Name* enter `quay.io/evanshortiss/summit-2020-rhmi-lab-nodejs-backend`.
.. Enter `{username}-traffic-api` in *Application Name* field.
.. Enter `{username}-traffic-api` in the *Name* field.
.. Under the *Resources* section Choose *DeploymentConfig*.
