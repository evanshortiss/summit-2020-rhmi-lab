...
In this section we will use the Red Hat API designer to explore a pre-built OpenAPI Specification which is used in the implementation of our API Server.
...
----
+
NOTE: If your username contains a hyphen, replace it with an underscore when using it to connect to the database, e.g instead of `test-user` use `test_user`.
...
----
+
NOTE: If your username contains a hyphen, you need to ensure the table in the query above uses an underscore. For example, change `INSERT INTO junction_status_test-user` to `INSERT INTO junction_status_test_user`.
...
+
NOTE: If your username contains a hyphen, replace it with an underscore when using it to connect to the database.
...
+
NOTE: Remember to replace hyphens in your username with underscores if necessary!
...
+
NOTE: Ask your lab administrator for a Google Maps API Key at this point, or get one of your own via the link:{https://console.cloud.google.com/google/maps-apis/overview}[Google Cloud Platform, window="_blank"].
