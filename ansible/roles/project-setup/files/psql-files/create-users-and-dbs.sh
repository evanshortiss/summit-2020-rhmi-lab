for NUM in {1..75}
do
  USER_USERNAME=$(printf "test_user%02d\n" $NUM)
  ...
  psql -d city-info -c "GRANT SELECT ON TABLE junction_info TO $USER_USERNAME;"
  psql -d city-info -c "GRANT SELECT ON TABLE meter_info TO $USER_USERNAME;"
  psql -d city-info -c "GRANT REFERENCES ON TABLE junction_info TO $USER_USERNAME;"
  psql -d city-info -c "GRANT REFERENCES ON TABLE meter_info TO $USER_USERNAME;"
  ...
  psql -d city-info -c "GRANT USAGE, SELECT ON SEQUENCE meter_status_${USER_USERNAME}_id_seq TO $USER_USERNAME;"
  psql -d city-info -c "GRANT USAGE, SELECT ON SEQUENCE junction_status_${USER_USERNAME}_id_seq TO $USER_USERNAME;"
done
